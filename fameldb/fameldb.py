...
class FameLeaderboard(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        self.monthlyleaderboardsaved = None
    ...
    async def update_embed(self):
        try:
            ...
            prevMONTHLY = None
            while True:
                self.monthlyleaderboardsaved = await self.monthlyleaderboard()
                if prevMONTHLY is not None:
                    await prevMONTHLY.delete()
                prevMONTHLY = await famechannel.send(embed=self.monthlyleaderboardsaved)
    ...
    async def monthlyleaderboard(self) -> discord.Embed:
        podium = ['🥇', '🥈', '🥉']


        embed=discord.Embed(title="TL Clash Royale Seasonal Fame Leaderboard", description='These are the top fame contributors from Threat Level Clans in the current river race season!', color=0x80ff00)
        embed.set_thumbnail(url="https://static.wikia.nocookie.net/clashroyale/images/9/9f/War_Shield.png/revision/latest?cb=20180425130200")
        embed.set_footer(text="Bot by: Threat Level Dev Team")
        ...
        for clantag in tl_clans:
            ...
            for member in data['clan']['participants']:
                if members.get(member['tag']) is not None:
                    members[member['tag']]['fame'] += member['fame']
                else:
                    members[member['tag']] = member
            index = data['sectionIndex']
            if(index != 1):
                url2 = 'https://api.clashroyale.com/v1/clans/%23{}/riverracelog'.format(clantag)
                async with aiohttp.ClientSession() as client:
                    async with client.get(url2, headers=self.headers) as resp:
                        if(resp.status != 200):
                            return discord.Embed(title='Clash Royale API Error', description = 'Clash Royale API is offline... data cannot be retreived :(')
                        data = await resp.json()
            for i in range(index-1):
                curlist = []
                for clan in data['items'][i]['standings']:
                    if(clan['clan']['tag'].strip('#') == clantag):
                        curlist = clan['clan']['participants']
                        break
        ...
        membs = [members[key] for key in members]
        
        ldb = sorted(membs, key=lambda member: -member['fame']) # sorts in descending order
    ...
    @commands.command()
    async def monthlyfame(self, ctx):
        """Get monthly fame leaderboard"""
        async with ctx.typing():
            if self.monthlyleaderboardsaved is not None:
                await ctx.send(embed=self.monthlyleaderboardsaved)
            else:
                self.monthlyleaderboardsaved = await self.monthlyleaderboard()
                await ctx.send(embed=self.monthlyleaderboardsaved)
