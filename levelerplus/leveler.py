...
class Leveler(commands.Cog):
    ...
    async def update_embed(self):
        try:
            ldbchannel = await self.bot.fetch_channel(844120574921539625)
            prev = None
            guild = await self.bot.fetch_guild(580455191376166943)
            while True:
                ld = await self.profiles._get_leaderboard(guild, monthly=True)
                emb = discord.Embed(title=_("Monthly Ranking"))
                for i in range(len(ld)):
                    cur = ld[i]
                    user = guild.get_member(cur["id"])
                    if user is None:
                        await self._reset_member(guild, cur["id"])
                    else:
                        txt = (
                            _("Level:")
                ...
                if prev is not None:
                    await prev.delete()
                prev = await ldbchannel.send(embed=emb)
